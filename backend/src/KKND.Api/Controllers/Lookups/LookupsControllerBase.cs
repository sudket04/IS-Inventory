...
{
    public sealed record UsageCount(string Label, Func<KkndDbContext, int, CancellationToken, Task<int>> CountAsync);

    public sealed record UsageResult(string Label, int Count);

    public sealed record SetActiveRequest(bool IsActive);

    protected readonly KkndDbContext Db;

    protected LookupsControllerBase(KkndDbContext db)
    {
        Db = db;
    }
